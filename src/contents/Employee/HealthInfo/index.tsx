...
const HealthInfo = ({ insertData, setInsertData, isEdit }: I_Props) => {
  ...
  const tableData = () => {
    return insertData?.healths.map(
      (child: { [key: string]: string }, i: number) => {
        return {
  ...
  return (
    <BodySTY>
      ...
      <LightBox
        ...
        title={modalOpen?.type == "create" ? "新增健康記錄" : "編輯健康記錄"}
